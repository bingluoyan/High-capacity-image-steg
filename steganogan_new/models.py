...
from steganogan_new.utils import bits_to_bytearray, bytearray_to_text, ssim, text_to_bits
...
class SteganoGAN(object):
    ...
    def fit(self, train, validate, epochs=5):
        ...
        for epoch in range(1, epochs + 1):
            ...
            if self.log_dir:
                ...
                writer.add_scalar('train/decoder_acc', self.fit_metrics['train.decoder_acc'], self.epochs)
                writer.add_scalar('train/decoder_loss', self.fit_metrics['train.decoder_loss'], self.epochs)
                writer.add_scalar('train/cover_score', self.fit_metrics['train.cover_score'], self.epochs)
                writer.add_scalar('train/generated_score', self.fit_metrics['train.generated_score'], self.epochs)
                writer.add_scalar('val/decoder_acc', self.fit_metrics['val.decoder_acc'], self.epochs)
                writer.add_scalar('val/decoder_loss', self.fit_metrics['val.decoder_loss'], self.epochs)
                writer.add_scalar('val/encoder_mse', self.fit_metrics['val.encoder_mse'], self.epochs)
                writer.add_scalar('val/cover_score', self.fit_metrics['val.cover_score'], self.epochs)
                writer.add_scalar('val/generated_score', self.fit_metrics['val.generated_score'], self.epochs)
                writer.add_scalar('val/ssim', self.fit_metrics['val.ssim'], self.epochs)
                writer.add_scalar('val/psnr', self.fit_metrics['val.psnr'], self.epochs)
                writer.add_scalar('val/bpp', self.fit_metrics['val.bpp'], self.epochs)
    ...
    def decode(self, image):
        ...
        bits = [int(bit) for bit in image.data.cpu().numpy().tolist()]
        #bits = bits_to_bytearray(bits).split(b'\x00\x00\x00\x00')
    ...
    def evaluate(self, dataloader,output_file=None):
        """
        Predict on the test dataset and returns several metrics.
        """
        self.critic.eval()
        self.encoder.eval()
        self.decoder.eval()
        ...
        for cover, _ in tqdm(dataloader, disable=not self.verbose):
            cover = cover.to(self.device)
            with torch.no_grad():
                generated, payload, decoded = self._encode_decode(cover, quantize=True)
                cover_score = self._critic(cover)
                generated_score = self._critic(generated)
                mse_loss_value = mse_loss(cover, generated)
                bce_loss_value = binary_cross_entropy_with_logits(decoded, payload)
                accuracy = (decoded >= 0.0).eq(payload >= 0.5).sum().float() / payload.numel()
                ssim_value = ssim(cover, generated)
                psnr_value = 10 * torch.log10(4 / mse_loss_value)
                mse_losses.append(mse_loss_value.item())
                decoder_losses.append(bce_loss_value.item())
                decoder_accs.append(accuracy.item())
                cover_scores.append(cover_score.item())
                generated_scores.append(generated_score.item())
                ssims.append(ssim_value.item())
                psnrs.append(psnr_value.item())
                bpp.append(self.data_depth * (2 * accuracy.item() - 1))
            
        metrics = {
            'mse_loss': sum(mse_losses) / len(mse_losses),
            'decoder_loss': sum(decoder_losses) / len(decoder_losses),
            'decoder_accuracy': sum(decoder_accs) / len(decoder_accs),
            'cover_score': sum(cover_scores) / len(cover_scores),
    ...
    def evaluate_image(self, input_image_path, output_image_path):
        """
        Predict on a pair of input and output images and return several metrics.
        Args:
        ...
        input_tensor = torch.FloatTensor(np.array(input_image)).permute(2, 0, 1).unsqueeze(0) / 127.5 - 1.0
        input_tensor = input_tensor.to(self.device)

        # Perform encoding and decoding
        with torch.no_grad():
            generated, payload, decoded = self._encode_decode(input_tensor, quantize=True)

        # Save generated output image
        if output_image_path is not None:
            generated_image = (generated[0].permute(1, 2, 0).detach().cpu().numpy() + 1.0) * 127.5
            generated_image = generated_image.astype('uint8')
            Image.fromarray(generated_image).save(output_image_path)

        # Calculate evaluation metrics
        mse_loss_value = mse_loss(input_tensor, generated)
        accuracy = (decoded >= 0.0).eq(payload >= 0.5).sum().float() / payload.numel()
        ssim_value = ssim(input_tensor, generated)
        psnr_value = 10 * torch.log10(4 / mse_loss_value)
